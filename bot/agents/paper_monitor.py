...
# Shadow mode RETIRED (July 2026 architecture audit). It existed to validate
# bot/exit_engine.py's decide_exit() against the live monitor BEFORE cutting
# over to it. That cutover never happened this way — SmartScalingExitManager
# was built instead as the real profit-side exit system, with a different,
# configurable scale-out ladder (28%@2x/22%@4.5x/12%@8x vs exit_engine.py's
# hardcoded 40%@2x/25%@5x). So the shadow had drifted into comparing live
# results against a THIRD, orphaned hypothetical nobody runs — /shadowstats
# was measuring something increasingly meaningless. Removed the per-tick
# _shadow_eval() call and the on-close _shadow_compare() call (zero
# production code still imports bot/exit_engine.py). The file + its 29 unit
# tests are left in place as an archived design reference — dead at runtime,
# zero cost, real historical value, not worth deleting outright.
